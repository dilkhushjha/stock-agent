...
function SectorCard({ sector, items, recommendations = [], rank }) {
  ...
  return <article className={`sector-card ${rank === 1 ? "sector-card-top" : ""}`}>
    ...
    <div className="sector-card-metrics"><div><span>NEWS</span><strong>{items.length}</strong><small>signals</small></div><div><span>CONFIDENCE</span><strong>{percent(avgConfidence)}</strong><small>average</small></div><div><span>5D MODEL</span><strong className={avgPrediction !== null && avgPrediction >= 0 ? "positive" : avgPrediction !== null ? "negative" : ""}>{pct(avgPrediction)}</strong><small>exposed stocks</small></div><div><span>HIGH IMPACT</span><strong>{highImpact}</strong><small>events</small></div></div>
    <div className="sector-card-effect"><span>CAUSE → EFFECT</span><p>{strongest?.real_world_effect || `News → real-world effect → ${label(sector)} earnings / demand → exposed companies → stock response.`}</p></div>
    <div className="sector-card-footer"><div><span>STOCKS UNDER WATCH</span><strong>{topStocks || "Exposure analysis pending"}</strong></div><div><span>PRIORITY</span><strong className={rank === 1 ? "sector-priority-high" : ""}>{rank === 1 ? "MOST ACTIVE" : rank <= 3 ? "WATCH" : "MONITOR"}</strong></div></div>
  </article>;
}
...
export default function SectorCards({ groups, recommendations }) {
  const ranked = useMemo(() => groups.map(([sector, items]) => {
    ...
    const high = items.filter((x) => String(x.impact || "").toUpperCase() === "HIGH").length;
