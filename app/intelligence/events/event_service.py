...
class EventIntelligenceService:
    ...
    def process_article(self, db: Session, article_id: int) -> dict:
        ...
        existing_event = db.scalar(
            select(MarketEvent).where(MarketEvent.news_id == article_id)
        )
        if existing_event:
            return {"status": "already_processed", "event_id": existing_event.id}
        ...
        if not result.get("is_market_relevant", True):
            article.is_processed = True
            db.commit()
            return {"status": "not_market_relevant", "event_id": None}

        existing = EventFusionEngine.find_related(
            db=db,
        ...
        event = MarketEvent(
            news_id=article.id,
            event_date=article.published_at or article.created_at,
