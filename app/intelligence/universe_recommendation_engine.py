...
class UniverseRecommendationEngine:
    ...
    def build(cls, db, limit: int = 10) -> list[dict]:
        ...
        stocks = db.scalars(
            select(Stock).where(Stock.is_active.is_(True), Stock.sector.is_not(None))
            .order_by(Stock.symbol).limit(cls.UNIVERSE_LIMIT)
        ).all()
        ...
        # Load fundamentals once so peer-relative valuation does not issue a query per stock.
        fundamentals_rows = db.scalars(select(CompanyFundamentals)).all()
        fundamentals_by_stock = {f.stock_id: f for f in fundamentals_rows}
        peers_by_sector = {}
        for f in fundamentals_rows:
            sector = str(getattr(f, "sector", None) or "").strip().upper()
            if sector:
                peers_by_sector.setdefault(sector, []).append(f)
        for stock in stocks:
            sector = str(stock.sector or "").strip().upper()
            if sector and sector not in peers_by_sector:
                peers_by_sector[sector] = []

        events = db.scalars(
            select(MarketEvent).where(MarketEvent.event_date >= cutoff)
            .order_by(MarketEvent.event_date.desc()).limit(500)
        ).all()
        ...
        for stock in stocks:
            try:
                ...
                # Sector-relative valuation: compare P/E and P/B with the peer median,
                # then fold that evidence into the fundamental score.
                fundamentals = fundamentals_by_stock.get(stock.id)
                if fundamentals:
                    relative = RelativeValuationEngine.assess(
                        stock, fundamentals, peers_by_sector.get(sector, [])
                    )
                    base_fundamental = float(item.get("fundamental_score") or 50)
                    relative_score = relative.score
                    blended_fundamental = 0.70 * base_fundamental + 0.30 * relative_score
                    item["fundamental_score"] = round(blended_fundamental, 1)
                    item["fundamental_relative_valuation_score"] = round(relative_score, 1)
                    item["relative_valuation"] = relative.as_dict()
                    item.setdefault("fundamentals", {})["relative_valuation"] = relative.as_dict()
                    item["fundamental_flags"] = list(dict.fromkeys(
                        (item.get("fundamental_flags") or []) + relative.flags
                    ))[:10]
                else:
                    item["fundamental_relative_valuation_score"] = None
                    item["relative_valuation"] = None
        ...
        candidates.sort(key=lambda x: (
            cls.PRIORITY_ORDER.get(x.get("priority"), 3), x.get("sector_rank", 999), -float(x.get("score") or 0)
        ))
    ...
    def _sector_stats(events):
        ...
        for sector, rows in grouped.items():
            ...
            score += min(14, positive * 4) - min(14, negative * 5) + confidence * 10 + recency * 8
            result[sector] = {"score": round(max(0, min(100, score)), 1), "count": len(rows), "high_impact": high,
                              "positive": positive, "negative": negative, "confidence": round(confidence, 3),
    ...
    def _reasons(item, ss):
        ...
        rv = item.get("relative_valuation") or {}
        if rv.get("peer_pe_median") is not None: reasons.append(f"P/E vs sector median: {float(rv.get('pe_premium_pct') or 0):+.0f}%")
        if item.get("predicted_5d") is not None: reasons.append(f"ML 5D: {float(item['predicted_5d']):+.2f}%")
        if opportunity.get("risk_reward") is not None: reasons.append(f"Estimated risk/reward: {float(opportunity['risk_reward']):.1f}x")
        return reasons[:8]
