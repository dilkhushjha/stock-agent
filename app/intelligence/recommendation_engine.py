...
class RecommendationEngine:
    ...
    def _market_stats(db, stock_id: int) -> dict:
        ...
        vol20 = (sum((x - sum(daily_returns[-20:]) / len(daily_returns[-20:])) ** 2 for x in daily_returns[-20:]) / 20) ** 0.5 * sqrt(252) * 100 if len(daily_returns) >= 20 else None
    ...
    def _evidence_score(db, event, news) -> float:
        count = RecommendationEngine._evidence_count(db, event, news)
        return min(100.0, 45 + count * 9)
    ...
    def _why_now(alert, prediction, market) -> str:
        ...
        if market.get("return_5d_pct") is not None:
            parts.append(f"Stock has moved {market['return_5d_pct']:+.2f}% over 5 sessions.")
        if market.get("volume_vs_20d_avg") is not None:
            parts.append(f"Latest volume is {market['volume_vs_20d_avg']:.1f}x its 20-session average.")
    ...
    def _invalidation(alert, prediction, market) -> str:
        ...
        if market.get("distance_from_52w_high_pct") is not None and market["distance_from_52w_high_pct"] > -2:
            return "Avoid chasing if price remains within roughly 2% of its 52-week high; thesis weakens if the catalyst is fully priced in."
